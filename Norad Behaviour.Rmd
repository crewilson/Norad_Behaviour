...
ggplot(Trials, aes(x=Ch)) + geom_histogram(binwidth=1)
...
T4 <- summarySE(Trials, measurevar="Nt", groupvars=c("bTrial"))
T4$var <-"Nt"
names(T4)[names(T4) == 'Nt'] <- 'Val'
T <- rbind(T1, T2, T3, T4)
rm("T1", "T2", "T3", "T4")
...
TT4 <- summarySE(Trials, measurevar="Nt", groupvars=c("block"))
TT4$var <-"Nt"
names(TT4)[names(TT4) == 'Nt'] <- 'Val'
TT <- rbind(TT1, TT2, TT3, TT4)
rm("TT1", "TT2", "TT3", "TT4")
...
ggplot(T, aes(x=bTrial,y=Val,colour=var)) + #What data to plot
    ...
    geom_errorbar(aes(ymin=Val-se, ymax=Val+se), width=.1) +
    geom_hline(aes(yintercept=0.333), colour="#990000", linetype="dashed") +
    ggtitle("Rewardeds touch proportion within block") +
    theme(plot.title = element_text(hjust = 0.5)) +
    ylab("Proportion") +
    xlab("Trials in block") +
    theme_bw() +
...
Trials$ShiftF <- as.factor(Trials$Shift)
    ggplot(Trials, aes(x=ShiftF, y=roll_sum)) + geom_boxplot() +
    ggtitle("Reward status when shifting") +
    ylab("Rewards in previous 5 trials") +
    xlab("Shift (1) or Stay") +
    theme_bw() 

    # Previous trial when shifting
    ggplot(Trials, aes(x=ShiftF, y=PrevRew)) + geom_boxplot() +
    ggtitle("Reward status when shifting") +
    ylab("Previously rewarded?") +
    xlab("Shift (1) or Stay") +
    theme_bw() 
    
#To here CREW 080322
# Shifts after COR and ERR
# Check the TRAP variable
# A count for that previous graph...
